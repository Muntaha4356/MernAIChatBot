      ...
      {message.role === "user" ? (
        <div className="flex items-start justify-end my-4 gap-2">
          ...
          border border-[#80609F]/30 rounded-md max-w-2xl">
            ...
            <span className='text-xs text-gray-400 dark:text-[#81A6C0]'>{moment(message.timestamp).fromNow()}</span>
          </div>
        ...
        dark:bg-[#57317C]/30 border border-[#80609F]/30 rounded-md my-4">
          {message.isImage ? (
            <img className='w-full max-w-md rounded-md mt-2' src={message.content} />
          ):(
