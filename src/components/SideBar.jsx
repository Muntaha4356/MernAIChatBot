...
const SideBar = ({isMenuOpened, setIsMenuOpened}) => {
  ...
  return (
    ...
    transition-all duration-500 max-md:absolute left-0 z-1 ${!isMenuOpened && 'max-md:-translate-x-full '}`}>
      {/* Logo */}
      ...
      <div onClick={()=>{navigate('/credits'); setIsMenuOpened(false);}} className='flex items-center gap-2 p-3 mt-4 border border-gray-300
      dark:border-white/15 rounded-md cursor-pointer hover:scale-103 transition-all'>
      ...
      {/* User Account  */}
      <div className='flex items-center gap-3 p-3 mt-4 border border-gray-300 justify-between
